...
int main() {
    ...
    BinTreeElementType items[] = {'P', 'R', 'O', 'C', 'E', 'D', 'U', 'R', 'E'};
    for (int i = 0; i < (int)(sizeof(items) / sizeof(BinTreeElementType)); i++) {
        BSTInsert(&root, items[i]);
    }

    BinTreeElementType minValue = MinBSTValue(root);
    BinTreeElementType maxValue = MaxBSTValue(root);
...
void BSTSearch(BinTreePointer Root, BinTreeElementType KeyValue, bool *Found,
